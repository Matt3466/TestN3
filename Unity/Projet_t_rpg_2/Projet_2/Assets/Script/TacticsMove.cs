...
public class TacticsMove : MonoBehaviour
{
    ...
    public void GetCurrentTile() // fonction qui permet de définir que la tuile sur laquelle est le personnage est la tuile "current"
    {
        currentTile = GetTargetTile(gameObject);
        currentTile.occupee = true;
    ...
    public Tile GetTargetTile(GameObject target)    //
    {
        ...
        if (Physics.Raycast(target.transform.position, -Vector3.up, out hit, 1))
        {
            ...
            ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////Debug.Log("TacticsMove, getTargetTile, tile = " + tile);
        }
        return tile;
    }
    ...
    public void RemoveOccupee()
    {
        currentTile.occupee = false;
    }
    ...
    public void MoveToTile(Tile tile)
    {
        ...
        if (next == null)
        {
            ...
            /////////////////////////////////////////////// Debug.Log("TacticsMove, MoveToTile, tile // " + this.name + " va sur cette tuile:" + tile);/////////////////////////////////////////////////////////////////////////////////////
    ...
    public void Move()
    {
        ...
        else
        {
            ...
            if (this.tag == "NPC")
            {
                if (!this.GetComponent<CombatNPC>().NPCKO)
                {
                    if (this.GetComponent<CombatNPC>().ActionAttaque)
                    {
                        this.GetComponent<CombatNPC>().NPCAttaque();
                        TurnManager.EndTurn();
                    }
                }
                else
            ...
            if (this.tag == "Player")
            {
                ...
                {
                    if (!GetComponent<CombatPlayer>().P1ContactAvecNPC1 && !GetComponent<CombatPlayer>().P1ContactAvecNPC2)
                    {
                        GetComponent<CombatPlayer>().FDT();
                        TurnManager.EndTurn();
                    }
                }
                if (this.name == "Player2")
                {
                    ...
                    if (!GetComponent<CombatPlayer>().P2ContactAvecNPC1 && !GetComponent<CombatPlayer>().P2ContactAvecNPC2)
                    {
                        GetComponent<CombatPlayer>().FDT();
                        TurnManager.EndTurn();
                    }
    ...
    public void BeginTurn()
    {
        ...
        if (this.tag == "NPC")
        {
            ...
            /*if (!this.GetComponent<CombatNPC>().NPCKO)
            {
                this.GetComponent<CombatNPC>().ActionAttaque = true;
                turn = true;
            }
            else
                TurnManager.EndTurn();*/
        ...
        if (this.tag == "Player")
        {
            if (!this.GetComponent<CombatPlayer>().PlayerKO)
            {
                turn = true;
            }
